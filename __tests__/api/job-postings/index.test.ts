...
describe('/api/job-postings', () => {
  ...
  describe('GET', () => {
    it('should return job postings ordered by created_at desc', async () => {
      global.mockPrisma.jobPosting.findMany.mockResolvedValueOnce(mockJobPostings)

      const request = createMockRequest('http://localhost:3000/api/job-postings')
    ...
    it('should handle database errors in GET', async () => {
      global.mockPrisma.jobPosting.findMany.mockRejectedValueOnce(new Error('Database error'))
  ...
  describe('POST', () => {
    ...
    it('should handle database errors in POST', async () => {
      global.mockPrisma.jobPosting.create.mockRejectedValueOnce(new Error('Database error'))
