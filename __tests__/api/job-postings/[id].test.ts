...
describe("/api/job-postings/[id]", () => {
  ...
  describe("GET", () => {
    it("should return job posting when found", async () => {
      global.mockPrisma.jobPosting.findUnique.mockResolvedValueOnce(
        mockJobPosting
      );

      const request = createMockRequest(
        "http://localhost:3000/api/job-postings/123"
      );
    ...
    it("should handle database errors in GET", async () => {
      global.mockPrisma.jobPosting.findUnique.mockRejectedValueOnce(
  ...
  describe("PUT", () => {
    ...
    it("should update job posting successfully", async () => {
      ...
      global.mockPrisma.jobPosting.update.mockResolvedValueOnce(
        updatedJobPosting
      );
    ...
    it("should handle database errors in PUT", async () => {
      global.mockPrisma.jobPosting.update.mockRejectedValueOnce(
        new Error("Database error")
      );
  ...
  describe("DELETE", () => {
    ...
    it("should handle database errors in DELETE", async () => {
      global.mockPrisma.jobPosting.delete.mockRejectedValueOnce(
        new Error("Database error")
      );
