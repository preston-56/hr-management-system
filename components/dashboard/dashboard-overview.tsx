...
import { Users, UserPlus, FileText, GraduationCap, Calendar, Award } from "lucide-react"
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Welcome back, {user?.firstName}! Here&apos;s your HR system overview</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{isLoading ? "..." : stats.openPositions}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{isLoading ? "..." : stats.trainingSessions}</div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-2">
            {canManageEmployees && (
              <Button className="w-full justify-start" variant="outline" onClick={() => setShowAddEmployee(true)}>
                <UserPlus className="mr-2 h-4 w-4" />
                Add New Employee
              </Button>
            )}
            <Button className="w-full justify-start" variant="outline" onClick={() => setShowLeaveRequest(true)}>
              <Calendar className="mr-2 h-4 w-4" />
              Process Leave Request
            </Button>
            {canApproveLeave && (
              <Button className="w-full justify-start" variant="outline" onClick={() => setShowPerformanceReview(true)}>
                <Award className="mr-2 h-4 w-4" />
                Start Performance Review
              </Button>
            )}
            <Button className="w-full justify-start" variant="outline" onClick={() => setShowScheduleTraining(true)}>
      ...
      {/* Dialog Components */}
      {canManageEmployees && <AddEmployeeDialog open={showAddEmployee} onOpenChange={setShowAddEmployee} />}
      <LeaveApplicationDialog open={showLeaveRequest} onOpenChange={setShowLeaveRequest} />
      {canApproveLeave && (
        <CreateAppraisalDialog open={showPerformanceReview} onOpenChange={setShowPerformanceReview} />
      )}
      <ScheduleTrainingDialog open={showScheduleTraining} onOpenChange={setShowScheduleTraining} />
    </div>
  )
}
